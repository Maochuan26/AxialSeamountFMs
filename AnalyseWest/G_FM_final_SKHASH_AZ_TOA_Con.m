...
%% input and output
load('/Users/mczhang/Documents/GitHub/FM3/02-data/F_Cl/F_final_AZ_TOF_V3_Con.mat');
Po=Po_Clu;clear Po_Clu;
tic;
   n=1;m=1;
for i=1:1000%length(unique([Po.Cluster])) 
    ...
    for j=1:length(ind_Cl)
        for k=1:length(fields) % Noise, Pamp, Samp;
           ...
           if isfield(Po(ind_Cl(j)).event, ['TOA', fields{k}]) 
            ...
            takeoff_uncertainty(n)=0;
            azimuth_uncertainty(n)=0;
            n=n+1;
