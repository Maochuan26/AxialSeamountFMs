...
pdf_file = ['/Users/mczhang/Documents/GitHub/FM3/03-output-graphics/' ...
    'G_HASH_Many_FMs_verticalV5.pdf'];
...
for reg=1:2
    ...
    for kp=1:length(project)
        ...
        if strcmp(define_region, 'West')
            ...
            %event([event.faultType] == 'U') = [];
            Felix([Felix.lat] < regions.West.Lat(1) | [Felix.lat] > regions.West.Lat(2)) = [];
            Felix([Felix.lon] < regions.West.Lon(1) | [Felix.lon] > regions.West.Lon(2)) = [];
        ...
        elseif strcmp(define_region, 'East')
            ...
            %event([event.faultType] == 'U') = [];
            Felix([Felix.lat] < regions.East.Lat(1) | [Felix.lat] > regions.East.Lat(2)) = [];
            Felix([Felix.lon] < regions.East.Lon(1) | [Felix.lon] > regions.East.Lon(2)) = [];
...
%        if reg==1
        ...
        for j = 1:length(depth_bins) - 1
            ...
            %X_Z_test_fissures;
            axis equal;
            figure;set(gcf,'Position',[2828         378        952         802])
            %scatter([Felixp.lat],[Felixp.depth],1);
            ...
            xlim([y1-1/10*y_length y2+1/10*y_length]);
            %scatter([Felixp.lat], [Felixp.depth], 10, [Felixp.depth], 'filled');
            ...
            for i = 1:length(eventp)
                ...
                h = plot_balloon(eventp(i).avfnorm, eventp(i).avslip, eventp(i).depth,eventp(i).lat,eventp(i).Mw*0.008, 1/60, eventp(i).color);
            ...
            hold off;
            set(gca,'FontSize',24);
            exportgraphics(gcf, pdf_file, 'Append', true,'Resolution', 300);
            close;
        end
    end
    close all;
